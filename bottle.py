...
class BaseRequest(object):
    ...
    @DictProperty('environ', 'bottle.request.form', read_only=True)
    def form(self):
    ...
    def params(self):
        """ A :class:`FormsDict` with the combined values of :attr:`query` and
            :attr:`form`. File uploads are stored in :attr:`files`. """
        params = FormsDict()
        for key, value in self.query.allitems():
            params[key] = value
        for key, value in self.form.allitems():
            params[key] = value
        return params
    ...
    def POST(self):
        """ The values of :attr:`form` and :attr:`files` combined into a single
    ...
    def __setitem__(self, key, value):
        ...
        if key == 'wsgi.input':
            todelete = ('body', 'form', 'files', 'params', 'post', 'json')
